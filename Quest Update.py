# -*- coding: utf-8 -*-
"""
Created on Tue Jan 21 17:45:09 2025
...
while dataset<2:
   ...
   base_directory= r"C:\Users\Simo\OneDrive - Tindall Corporation\Desktop\MYPR vids NO TOUCH\Quest Routings"
   Missing_entrysheets=[]
   ...
   for index, row in Master_ProdTable.iterrows(): 
     ...
     masterrow_entrydata=np.zeros((116,)) # 116 is the number of elements to read from entry sheet
     ...
     for piece in filter_criterion:
           ...
           for idx, entry_row in entrysheet_data.iterrows():
               ...
               if idx<6 : # 6 is row index where entry sheet data actually starts#####
                   continue
            
               
               if pd.isna(entry_row.iloc[2]): 
                   entrysheet_data.iloc[idx, 2]=0
                   entrydata_list.append(0)
              
               else:    
                 try:
                   entrydata_list.append(float(entrysheet_data.iloc[idx,2])) #append entry sheet data into a list verify location
                 except (ValueError, TypeError):
                    
                  # If conversion fails, zero is appended
                   entrydata_list.append(0)
   ...
   #Review  of output of data manipulation
...
model = keras.Sequential([
    layers.Dense(256, activation='relu',kernel_initializer='he_uniform', input_shape=(116,)),  # Input layer with 256 features
